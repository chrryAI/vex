...
if (cluster.isPrimary) {
  ...
  let threads = parseInt(process.argv.find((x) => x.startsWith("--threads="))?.split("=")?.[1], 10);
  if (Number.isNaN(threads))
    try {
      threads = parseInt(fs.readFileSync(join(__dirname, ".threads"), "utf8"), 10);
    } catch {
      threads = Math.min(12, os.cpus().length) - 4;
      log.warning(
        "test262",
        `no --threads=n arg or .threads file found, using ${threads} as cautious default (min(12, threads) - 4)`,
      );
      log.warning(
        "test262",
        "please specify via either method to make test262 runs potentially much faster! (ask for tuning advice)",
      );
    }
  ...
  const lastCommitResults = minimal
    ...
    : execSync(`git log -200 --pretty=%B`)
        ...
        .find(
          (x) =>
            x.startsWith("test262: 1") ||
            x.startsWith("test262: 2") ||
            x.startsWith("test262: 3") ||
            x.startsWith("test262: 4") ||
            x.startsWith("test262: 5") ||
            x.startsWith("test262: 6"),
        )
        .split("|")
        .map((x) => parseFloat(x.split("(")[0].trim().split(" ").pop().trim().replace("%", "")));

  if (!resultOnly) process.stdout.write("\u001b[90mreading tests...\u001b[0m");

  const preludes = fs
    .readFileSync(join(__dirname, "harness.js"), "utf8")
    .split("///")
    .reduce((acc, x) => {
  ...
  const _onlyTrackCompilerErrors = process.argv.includes("--compiler-errors-only");
  ...
  const runIconTable = plainResults
    ? [
        "pass:",
        "todo:",
        "wasm compile error:",
  ...
  const noAnsi = (s) => s.replace(new RegExp(`${String.fromCharCode(27)}\\[[0-9]+m`, "g"), "");
      ...
      if (!resultOnly && !logErrors) {
        ...
        if (allTests) {
          ...
          if (percent > lastPercent) {
            const tab =
              `  \u001b[1m${spinner[spin++ % 4]} ${percent.toFixed(1)}%\u001b[0m    ` +
              table(
                false,
                total,
                passes,
            ...
            process.stdout.write(
              (lastPercent !== 0 ? `\u001b[2F\u001b[0J` : `\r${" ".repeat(100)}\r`) +
                bar(
                  ...
                  passes,
                  fails,
                  runtimeErrors + timeouts,
                  compileErrors + wasmErrors,
                  0,
                ) +
                "\n" +
                tab +
          ...
          process.stdout.write(
            `\r${" ".repeat(100)}\r\u001b[90m${percent.toFixed(0).padStart(4, " ")}% |\u001b[0m \u001b[${pass ? "92" : result === 4 ? "93" : result === 5 ? "90" : "91"}m${runIconTable[result]} ${file}\u001b[0m\n`,
          );

          if (threads === 1 && tests[i + 1]) {
            const nextFile = tests[i + 1].file;
            process.stdout.write(
              `\u001b[90m${percent.toFixed(0).padStart(4, " ")}% | ${nextFile}\u001b[0m`,
            );
          }
        }
  ...
  if (resultOnly) {
    ...
    console.log(
      table(true, total, passes, fails, runtimeErrors, wasmErrors, compileErrors, timeouts),
    );
    process.exit();
  }
  ...
  console.log(
    `\u001b[1m${whatTests || "test262"}: ${passes}/${total} passed - ${percent.toFixed(2)}%${whatTests === "" && percentChange !== 0 ? ` (${percentChange > 0 ? "+" : ""}${percentChange.toFixed(2)})` : ""}\u001b[0m \u001b[90m(${togo(nextMinorPercent)}, ${togo(nextMajorPercent)})\u001b[0m`,
  );
  const tab = table(
    whatTests === "",
    total,
  ...
  if (trackErrors) {
    ...
    for (const x of Object.keys(errorsByClass).sort(
      (a, b) => errorsByClass[b] - errorsByClass[a],
    )) {
  ...
  if (profile) {
    const longestTests = Object.keys(perTestProfile)
      .sort((a, b) => perTestProfile[b] - perTestProfile[a])
      .slice(0, 10);
    ...
    for (const x of longestTests) {
      ...
      console.log(
        `${x}${" ".repeat(longestTestName - x.length)}\x1B[90m│\x1B[0m \x1B[1m${(perTestProfile[x] / 1000).toFixed(2)}s\x1B[0m`,
      );
    }
    ...
    for (const x of ["total", "parse", "codegen", "opt", "assemble", "instantiate", "execute"]) {
      ...
      console.log(
        `${x}\x1B[90m: \x1B[0m\x1B[1m${((y / total) * 100).toFixed(0)}%\x1B[0m (${(y / 1000 / threads).toFixed(2)}s)`,
      );
    }
    console.log();
  }
  ...
  process.on("message", (i) => {
    ...
    if (!pass) {
      ...
      if (
        trackErrors &&
        error &&
        (!onlyTrackCompilerErrors ||
          (stage === 0 && errorName !== "CompileError" && errorName !== "SyntaxError"))
      ) {
    ...
    if (logErrors) {
      ...
      console.log(
        `\u001b[${pass ? "92" : "91"}m${runIconTable[out & 0b1111]} ${test.file}\u001b[0m${e ? `\n${e}` : ""}`,
      );
    }
