...
if (cluster.isPrimary) {
  ...
  const lastCommitResults = minimal
    ...
    : execSync(`git log -200 --pretty=%B`)
        ...
        .map((x) =>
          parseFloat(
            x.split("(")[0].trim().split(" ").pop().trim().replace("%", ""),
          ),
        )
  ...
  if (!resultOnly)
    process.stdout.write(
      `\r${" ".repeat(60)}\r\u001b[90mcaching tests to tmp...\u001b[0m`,
    )

  fs.writeFileSync(workerDataPath, JSON.stringify(tests))
  if (!resultOnly)
    process.stdout.write(
      `\r${" ".repeat(60)}\r\u001b[90mstarting ${threads} runners...\u001b[0m`,
    )
  ...
  const nextMinorPercent = parseFloat(
    (Math.floor(percent * 10) / 10 + 0.1).toFixed(1),
  )
  const nextMajorPercent = Math.floor(percent) + 1

  const togo = (next) =>
    `${Math.floor((total * next) / 100 - passes)} to go until ${next}%`
  ...
  if (trackErrors) {
    ...
    for (const x of [...errors.keys()].sort(
      (a, b) => errors.get(a) - errors.get(b),
    )) {
