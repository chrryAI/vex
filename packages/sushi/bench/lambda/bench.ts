...
async function runLambdaFunction(functionName: string, iterations: number) {
  const benchmarkStartTime = Date.now()
  const results: TimingResult[] = []

  console.log(`[DEBUG] Function name: ${functionName}`)
  console.log(
    `[DEBUG] Benchmark start time: ${benchmarkStartTime} (${new Date(benchmarkStartTime).toISOString()})`,
  )
  ...
  for (let i = 0; i < iterations; i++) {
    try {
      ...
      if (updateResult.code !== 0) {
        throw new Error(
          `Function configuration update failed: ${updateResult.stderr}`,
        )
      }
      ...
      if (initDuration > 0 || duration > 0) {
        ...
        results.push(result)
        console.log(
          `Iteration ${i + 1}: Init ${initDuration}ms, Duration ${duration}ms, Billed ${billedDuration}ms, Memory ${maxMemoryUsed}MB`,
        )
      } else {
  ...
  if (results.length === 0) {
    console.log(
      `[ERROR] No timing data collected. This suggests the function invocations failed.`,
    )
    return
  }
  ...
  const validResults = results.filter(
    (r) => r.initDuration > 0 || r.duration > 0,
  )
  ...
  if (failedResults.length > 0) {
    console.log(
      `[WARNING] ${failedResults.length} iterations failed to parse timing data`,
    )
  }

  if (validResults.length === 0) {
    console.log(
      `[ERROR] No valid timing data found. Check if the Lambda function outputs timing information.`,
    )
  ...
  const initDurations = validResults
    .map((r) => r.initDuration)
    .filter((d) => d > 0)
  ...
  if (initDurations.length === 0) {
    console.log(
      `[WARNING] No Init Duration data found. This suggests the function may not be experiencing cold starts.`,
    )
  }
  ...
  if (durations.length === 0) {
    console.log(
      `[WARNING] No Duration data found. This suggests the function may not be outputting execution time.`,
    )
  }
  ...
  if (initDurations.length > 0) {
    initMean =
      initDurations.reduce((acc, val) => acc + val, 0) / initDurations.length
    initStddev = Math.sqrt(
      initDurations.reduce((acc, val) => acc + (val - initMean) ** 2, 0) /
        initDurations.length,
    )
  }
  ...
  if (durations.length > 0) {
    ...
    durationStddev = Math.sqrt(
      durations.reduce((acc, val) => acc + (val - durationMean) ** 2, 0) /
        durations.length,
    )
  }

  // Calculate Billed Duration statistics if available
  const billedDurations = validResults
    .map((r) => r.billedDuration)
    .filter((d) => d > 0)
  let billedDurationMean = 0
  let billedDurationStddev = 0
  if (billedDurations.length > 0) {
    billedDurationMean =
      billedDurations.reduce((acc, val) => acc + val, 0) /
      billedDurations.length
    billedDurationStddev = Math.sqrt(
      billedDurations.reduce(
        (acc, val) => acc + (val - billedDurationMean) ** 2,
        0,
      ) / billedDurations.length,
    )
  }
  ...
  const filteredBilledDurations =
    billedDurations.length > 0
      ? billedDurations.filter(
          (duration) =>
            Math.abs(duration - billedDurationMean) < 2 * billedDurationStddev,
        )
      : []
