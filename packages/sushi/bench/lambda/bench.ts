...
async function runLambdaFunction(functionName: string, iterations: number) {
  ...
  if (billedDurations.length > 0) {
    billedDurationMean =
      billedDurations.reduce((acc, val) => acc + val, 0) / billedDurations.length;
    billedDurationStddev = Math.sqrt(
      billedDurations.reduce((acc, val) => acc + (val - billedDurationMean) ** 2, 0) /
  ...
  const _filteredInitDurations =
    initDurations.length > 0
      ? initDurations.filter((duration) => Math.abs(duration - initMean) < 2 * initStddev)
      : [];

  const _filteredDurations =
    durations.length > 0
      ? durations.filter((duration) => Math.abs(duration - durationMean) < 2 * durationStddev)
      : [];
