...
const running = new Uint32Array(1024) // Stores indices into samplesFunc/Start/End arrays
let runningIdx = 0

// --- Profiling Hooks ---
createImport("profile1", [Valtype.i32], 0, (f) => {
  // pre-call
...
createImport("profile2", 0, 0, () => {
  ...
  if (now - lastProgressUpdate > progressUpdateIntervalMs) {
    ...
    process.stdout.write(
      output + " ".repeat(Math.max(0, termWidth - output.length - 1)),
    )
  }
})
  ...
  for (const x of funcs) {
    ...
    for (let i = 0; i < w.length; i++) {
      if (w[i][0] === Opcodes.call) {
        ...
        w.splice(i, 0, number(f, Valtype.i32), [
          Opcodes.call,
          importedFuncs.profile1,
        ])
...
process.stdout.write(
  "\r" +
    initialMsg +
    " ".repeat(Math.max(0, termWidthInitial - initialMsg.length - 1)),
)
...
console.log(
  `Filtered out ${samplesBelowThreshold} samples shorter than ${minSampleDurationMs}ms.`,
)
...
function buildHierarchy(samples) {
  ...
  const stack = [
    { node: root, startTime: root.startTime, endTime: root.endTime },
  ...
  for (const sample of samples) {
    ...
    while (
      stack.length > 1 &&
      (stack[stack.length - 1].startTime > sampleStartTime ||
        stack[stack.length - 1].endTime < sampleEndTime)
    ) {
      stack.pop()
    }
...
const { id } = await (
  await fetch("https://profile.porffor.dev", {
    method: "POST",
    headers: {
