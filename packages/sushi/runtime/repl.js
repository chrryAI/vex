...
try {
  ...
  if (repl.REPLServer.prototype.defineCommand == null) throw "mock node:repl detected";
} catch {
...
const run = (source, _context, _filename, callback, run = true) => {
  ...
  try {
    ...
    console.log(
      util.inspect(ret, false, 2, true),
      value != null ? `\x1B[34m\x1B[3m(value: ${value}, type: ${TYPE_NAMES[type]})\x1B[0m` : "",
    );
