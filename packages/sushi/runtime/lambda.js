...
const outputZip = args.find(
  (x) => x[0] !== "-" && !x.includes("=") && x !== sourceFile,
)
...
if (!compiler) {
  try {
    ...
    log.warning(
      "lambda",
      `musl-gcc is recommended for Lambda as it noticably improves deployed performance. install it or explicitly specify a compiler via CC to hide this warning. defaulting to gcc.`,
    )
    compiler = "gcc"
  }
}
...
if (Prefs.d) {
  execSync(
    `${compiler} -lm -O0 -g -flto -march=x86-64-v3 -o ${binaryPath} ${cPath}`,
    { stdio: "inherit" },
  )
} else {
  execSync(
    `${compiler} ${compiler === "musl-gcc" ? "-static" : ""} -lm -O3 -fomit-frame-pointer -s -flto -march=x86-64-v3 -ffunction-sections -fdata-sections -Wl,--gc-sections -o ${binaryPath} ${cPath}`,
    { stdio: "inherit" },
  )
