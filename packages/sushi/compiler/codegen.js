...
const getNodeType = (scope, node) => {
  ...
  if (guess != null)
    out.guess =
      typeof guess === "number" ? [number(guess, Valtype.i32)] : guess;
...
const setLocalWithType = (
  ...
  const local = isGlobal
    ? globals[name]
    : (scope.locals[name] ?? { idx: name });
  const out = Array.isArray(decl)
    ? decl
    : generate(scope, decl, isGlobal, name);
...
const generateIf = (scope, decl) => {
  ...
  if (decl.alternate) {
    ...
    out.push([Opcodes.else], ...generate(scope, decl.alternate), [
      Opcodes.drop,
    ]);
    inferBranchEnd(scope);
...
const printStaticStr = (scope, str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    out.push(number(str.charCodeAt(i)), [
      Opcodes.call,
      importedFuncs.printChar,
    ]);
  }
        ...
        if (type) {
          ...
          if (
            ...
            if (
              ...
              if (typeAnno.type === TYPES.number)
                types.push(TYPES.numberobject);
              if (typeAnno.type === TYPES.string)
                types.push(TYPES.stringobject);
