...
import { bold, log, underline } from "./log.js"
import opt from "./opt.js"
import parse from "./parse.js"
import * as pgo from "./pgo.js"
import { PageSize, Valtype } from "./wasmSpec.js"
import "./prefs.js"
...
const progressStart = (msg) => {
  ...
  const log = (extra, after) => {
    ...
    process.stdout.write(
      `\r\u001b[2m${" ".repeat(60)}\r${" ".repeat(12 - pre.length)}${pre}  ${msg}${after ?? ""}\u001b[0m`,
    )
  }
  log()
...
const progressDone = (msg, start) => {
  if (globalThis.onProgress)
    return globalThis.onProgress(msg, performance.now() - start)

  clearInterval(progressInterval)

  const timeStr = (performance.now() - start).toFixed(0)
  console.log(
    `${process.stdout.isTTY ? `\r${" ".repeat(60)}\r` : ""}\u001b[2m${" ".repeat(10 - timeStr.length)}${timeStr}ms\u001b[0m  \u001b[92m${msg}\u001b[0m`,
  )
  progressLines++
}
const progressClear = () => {
...
export default (code, module = Prefs.module) => {
  ...
  if (globalThis.compileCallback)
    globalThis.compileCallback({
      funcs,
      globals,
      tags,
      exceptions,
  ...
  if (Prefs.pgo) {
    if (Prefs.pgoLog) {
      ...
      console.log(
        `PGO size diff: ${oldSize - newSize} bytes (${oldSize} -> ${newSize})\n`,
      )
    } else {
      pgo.run({ funcs, globals, tags, exceptions, pages, data })
      opt(funcs, globals, pages, tags, exceptions)
  ...
  if (Prefs.cyclone) {
    if (Prefs.cycloneLog) {
      ...
      console.log(
        `cyclone size diff: ${oldSize - newSize} bytes (${oldSize} -> ${newSize})\n`,
      )
    } else {
      for (const x of funcs) {
        cyclone(x, globals)
  ...
  if (Prefs.builtinTree) {
    ...
    if (typeof Prefs.builtinTree === "string")
      data = data.filter((x) => x.includes.has(Prefs.builtinTree))

    const funcsByName = funcs.reduce((acc, x) => {
      acc[x.name] = x
      return acc
    }, {})
  ...
  if (target === "wasm" && outFile) {
    ...
    if (logProgress) {
      ...
      progressClear()
      console.log(
        `\u001b[2m[${total.toFixed(0)}ms]\u001b[0m \u001b[32mcompiled ${globalThis.file} \u001b[90m->\u001b[0m \u001b[92m${outFile}\u001b[90m (${(fs.statSync(outFile).size / 1000).toFixed(1)}KB)\u001b[0m`,
      )
    }
  ...
  if (target === "native") {
    ...
    const args = [
      ...compiler,
      "-xc",
      "-", // use stdin as c source in
      "-o",
      outFile ?? (process.platform === "win32" ? "out.exe" : "out"), // set path for output
    ...
    if (logProgress)
      progressStart(`compiling C to native (using ${compiler})...`)
    ...
    if (logProgress)
      progressDone(`compiled C to native (using ${compiler})`, t5)
