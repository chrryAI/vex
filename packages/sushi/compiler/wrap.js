...
const _fs = typeof process?.version !== "undefined" ? await import("node:fs") : undefined;
  ...
  switch (type) {
    ...
    case TYPES.object: {
      ...
      for (let i = 0; i < size; i++) {
        ...
        if (Prefs.d) {
          const readMem = (ptr, size) =>
            [...read(Uint8Array, memory, ptr, size)]
...
\x1B[0m`);
    ...
    case TYPES.string: {
      ...
      return Array.from(read(Uint16Array, memory, value + 4, length))
        .map((x) => String.fromCharCode(x))
        .join("");
    }
    ...
    case TYPES.bytestring: {
      const length = read(Uint32Array, memory, value, 1)[0];
      return Array.from(read(Uint8Array, memory, value + 4, length))
        .map((x) => String.fromCharCode(x))
        .join("");
    }

    case TYPES.stringobject:
      return new String(porfToJSValue({ memory, funcs, pages }, value, TYPES.string));
    ...
    case TYPES.promise: {
      const [result, _state, _fulfillReactions, _rejectReactions] = porfToJSValue(
        { memory, funcs, pages },
        value,
        TYPES.array,
        4,
    ...
    case TYPES.test262error: {
      const message = porfToJSValue(
        { memory, funcs, pages },
        read(Uint32Array, memory, value, 1)[0],
        read(Uint8Array, memory, value + 4, 1)[0],
      );
        ...
        while (true) {
          ...
          switch (opcode) {
            ...
            case 0x0b: {
              ...
              i += 2;
              console.log(
                "\x1b[93mpositive lookahead\x1b[0m",
                target,
                `\x1b[2m(${i + target - 3})\x1b[0m`,
              );
              break;
            }
            case 0x0c: {
              ...
              i += 2;
              console.log(
                "\x1b[93mnegative lookahead\x1b[0m",
                target,
                `\x1b[2m(${i + target - 3})\x1b[0m`,
              );
              break;
            ...
            case 0x21: {
              ...
              console.log(
                "\x1b[93mfork\x1b[0m",
                branch1,
                `\x1b[2m(${i + branch1 - 5})\x1b[0m`,
                branch2,
                `\x1b[2m(${i + branch2 - 5})\x1b[0m`,
              );
              break;
            }
...
export default (source, module = undefined, print = (str) => process.stdout.write(str)) => {
  ...
  const { wasm, funcs, globals, tags, exceptions, pages, c } =
    typeof source === "object" ? source : compile(source, module);

  times.push(performance.now() - t1);
  if (Prefs.profileCompiler && !globalThis.onProgress)
    console.log(`\u001b[1mcompiled in ${times[0].toFixed(2)}ms\u001b[0m`);
  ...
  const printBacktrace = (middleIndex, func, funcs, globals, exceptions) => {
    ...
    const disasm = disassemble(
      func.wasm.slice(min, max),
      func.name,
      0,
      func.locals,
      func.params,
  ...
  const backtrace = (funcInd, blobOffset) => {
    if (
      funcInd == null ||
      blobOffset == null ||
      Number.isNaN(funcInd) ||
      Number.isNaN(blobOffset) ||
      Prefs.backtrace === false
    )
      return false;
      ...
      try {
        ...
        if (rawValues)
          return {
            value: ret[0],
            type: ret[1],
            js: porfToJSValue({ memory, funcs, pages }, ret[0], ret[1]),
          };
