...
const read = (ta, memory, ptr, length) => {
  ...
  return new ta(
    memory.buffer.slice(ptr, ptr + length * ta.BYTES_PER_ELEMENT),
    0,
    length,
  )
}
...
\x1B[0m`)
    ...
    case TYPES.stringobject:
      return new String(
        porfToJSValue({ memory, funcs, pages }, value, TYPES.string),
      )
    ...
    case TYPES.dataview: {
      ...
      const buffer = memory.buffer.slice(
        bufferPtr + 4,
        bufferPtr + 4 + bufferLen,
      )
        ...
        while (true) {
          ...
          switch (opcode) {
            ...
            case 0x31: {
              const index = bc[i++]
              console.log(
                `\x1b[2m${opcode === 0x30 ? "start" : "end"} capture ${index}\x1b[0m`,
              )
              break
            }
          }
