...
registerInternalType(
  "ByteString",
  ["iterable", "length"],
  TYPES.string | TYPE_FLAGS.parity,
)
...
if (Prefs.largestTypes) {
  ...
  const logType = (label, val, key) =>
    console.log(
      `${label}    ${key} - ${val} (0x${val.toString(16)}, 0b${val.toString(2).padStart(8, "0")})`,
    )

  const largestUnflagged = largestType(
    typeVals.map((x) => x & 0b00111111),
    typeKeys,
  )
