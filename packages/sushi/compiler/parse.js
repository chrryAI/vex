...
const loadParser = async (fallbackParser = "acorn", forceParser) => {
  ...
  const mod = await import(
    (globalThis.document ? "https://esm.sh/" : "") + parser
  )
...
if (types && !["@babel/parser", "hermes-parser", "oxc-parser"].includes(parser))
  log.warning(
    "parse",
    `passed -parse-types with a parser (${parser}) which does not support`,
  )
