...
export default ({ funcs, globals, data, pages }) => {
  if (Prefs.secure)
    log.warning(
      "2c",
      `native/c targets are not sandboxed or proven to be safe (--secure)`,
    )
  ...
  const sanitize = (str) => {
    ...
    return str.replace(/[^0-9a-zA-Z_]/g, (_) =>
      codeToSanitizedStr(_.charCodeAt(0)),
    )
  }
  ...
  const includes = new Map(),
    unixIncludes = new Map(),
    winIncludes = new Map()
  const prepend = new Map(),
    prependMain = new Map()
  ...
  if (pages.size > 0) {
    ...
    prepend.set(
      "_memory",
      `char* _memory; u32 _memoryPages = ${Math.ceil((pages.size * pageSize) / PageSize)};\n`,
    )
    prependMain.set(
      "_initMemory",
  ...
  if (activeData.length > 0) {
    const dataOffset = (x) =>
      pages.allocs.get(x.page) ?? pages.get(x.page) * pageSize
    if (Prefs["2cMemcpy"]) {
      prependMain.set(
        "_data",
        activeData
          .map(
            (x) =>
              `memcpy(_memory + ${dataOffset(x)}, (unsigned char[]){${x.bytes.join(",")}}, ${x.bytes.length});`,
          )
          .join("\n  "),
      )
    } else {
        ...
        activeData
          .map((x) =>
            x.bytes.reduce(
              (acc, y, i) =>
                acc +
                (y === 0 ? "" : `_memory[${dataOffset(x) + i}]=(u8)${y};`),
              "",
            ),
          )
          .join("\n  "),
  ...
  const _line = (str, semi = true) => (out += `${str}${semi ? ";" : ""}\n`)
  const lines = (lines) => {
  ...
  const cify = (f) => {
    ...
    let depth = 1
    let brDepth = 0
    const line = (str, semi = true) =>
      (out += `${" ".repeat((depth + brDepth) * 2)}${str}${semi ? ";" : ""}\n`)
    const lines = (lines) => {
      for (const x of lines) {
    ...
    const localKeys = Object.keys(f.locals)
      .sort((a, b) => f.locals[a].idx - f.locals[b].idx)
      .slice(f.params.length)
      .sort((a, b) => f.locals[a].idx - f.locals[b].idx)
    ...
    for (let _ = 0; _ < f.wasm.length; _++) {
      ...
      if (i[0] === null && i[1] === "c") {
        ...
        let c = i[2]
        c = c.replace(
          /^\s*(?:(?:inline|static)\s+)?\w+\s+\*?\s*\w+\s*\(.*?\)\s* {([\w\W]*?)\n}/gm,
          (_) => {
            prepend.set(_, _)
            return ""
          },
      ...
      switch (i[0]) {
        ...
        case Opcodes.return: {
          ...
          line(
            `return${returns ? ` (struct ReturnValue){ ${removeBrackets(a)}, ${removeBrackets(b)} }` : ""}`,
          )
          break
        }
        ...
        case Opcodes.call: {
          const func = funcs.find((x) => x.index === i[1])
          if (!func) {
...
extern ${importFunc.returns.length > 0 ? CValtype[importFunc.returns[0]] : "void"} ${name}(${importFunc.params.map((x) => CValtype[x]).join(", ")});`,
            ...
            switch (importFunc.name) {
              case "print":
                line(
                  `printf("${valtype === "f64" ? "%.15g" : "%i"}", ${vals.pop()})`,
                )
...
f64 _time_out${id} = (f64)_ts${id}.tv_sec * 1000.0 + (f64)_ts${id}.tv_nsec / 1.0e6;`,
          ...
          for (let j = 0; j < func.params.length; j++)
            args.unshift(removeBrackets(vals.pop()))
          ...
          if (func.returns.length > 0) {
            if (func.returnType != null) {
              ...
              const id = retTmpId++
              line(
                `const struct ReturnValue _${id} = ${name}(${args.join(", ")})`,
              )
        ...
        case Opcodes.call_indirect:
          ...
          if (Prefs.d)
            log.warning(
              "2c",
              `unimplemented op: ${invOpcodes[i[0]]} \x1b[2m(${f.name})`,
            )
          vals.pop()
          vals.push("0", "0")
          break
        ...
        case Opcodes.br: {
          ...
          if (ret !== Blocktype.void)
            line(`_r${brs[brDepth - i[1] - 1]} = ${removeBrackets(vals.pop())}`)
        ...
        case Opcodes.br_if: {
          ...
          if (ret !== Blocktype.void)
            line(
              `_r${brs[brDepth - i[1] - 1]} = ${removeBrackets(vals.at(-1))}`,
            )
          line(`goto j${brs[brDepth - i[1] - 1]}`)
          depth--
          line(`}`, false)
        ...
        case Opcodes.i32_rotl: {
          ...
          line(
            `const i32 _out${id} = (_x${id} << _shift${id}) | (_x${id} >> (32 - _shift${id}))`,
          )
          vals.push(`_out${id}`)
          break
        }
        ...
        case Opcodes.memory_grow: {
          ...
          line(
            `memset(_memory + _oldPages${id} * ${PageSize}, 0, (_memoryPages - _oldPages${id}) * ${PageSize})`,
          )
        ...
        default:
          if (CMemFuncs[i[0]]) {
            ...
            if (!prepend.has(name)) {
              prepend.set(
                name,
                `${func.returns || "void"} ${name}(i32 align, i32 offset, ${func.args.map((x, i) => `${func.argTypes[i]} ${x}`).join(", ")}) {\n  ${func.c.replaceAll("\n", "\n  ")}\n}\n`,
              )
            }
            ...
            if (func.returns !== false) {
              vals.push(
                `${name}(${immediates[0]}, ${immediates[1]}, ${args.join(", ")})`,
              )
            } else
              line(
                `${name}(${immediates[0]}, ${immediates[1]}, ${args.join(", ")})`,
          ...
          if (Prefs.d)
            log.warning(
              "2c",
              `unimplemented op: ${invOpcodes[i[0]] ?? invOpcodes[`${i[0]},${i[1]}`]} \x1b[2m(${f.name})`,
            )
      }
  ...
  prepend.set(
    "func decls",
    funcs
      .filter((x) => x.name !== "#main" && cified.has(x.name))
      .map((f) => {
        const returns = f.returns.length > 0
        const typedReturns = f.returnType == null
        ...
        return `${!typedReturns ? (returns ? CValtype[f.returns[0]] : "void") : "struct ReturnValue"} ${shouldInline ? "inline " : ""}${ffiFuncs[f.name] ? "(*" : ""}${sanitize(f.name)}${ffiFuncs[f.name] ? ")" : ""}(${rawParams(
          f,
        )
          .map((x, i) => `${CValtype[x]} ${invLocals[i]}`)
          .join(", ")});`
      })
      .join("\n"),
  )
...
int main(void) {
  ...
  const makeIncludes = (includes) =>
    [...includes.keys()].map((x) => `#include <${x}>\n`).join("")
  out =
    platformSpecific(
      makeIncludes(winIncludes),
      makeIncludes(unixIncludes),
      false,
    ) +
    "\n" +
