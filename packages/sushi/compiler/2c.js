...
export default ({ funcs, globals, data, pages }) => {
  if (Prefs.secure)
    log.warning("2c", `native/c targets are not sandboxed or proven to be safe (--secure)`);
    ...
    if (Prefs["2cMemcpy"]) {
      ...
      prependMain.set(
        ...
        activeData
          .map((x) =>
            x.bytes.reduce(
              (acc, y, i) => acc + (y === 0 ? "" : `_memory[${dataOffset(x) + i}]=(u8)${y};`),
              "",
            ),
          )
          .join("\n  "),
...
f64 _time_out${id} = (f64)_ts${id}.tv_sec * 1000.0 + (f64)_ts${id}.tv_nsec / 1.0e6;`,
        ...
        case Opcodes.call_indirect:
          // stub
          if (Prefs.d)
            log.warning("2c", `unimplemented op: ${invOpcodes[i[0]]} \x1b[2m(${f.name})`);
          vals.pop();
          vals.push("0", "0");
          break;
        ...
        case Opcodes.br_if: {
          ...
          depth++;
          if (ret !== Blocktype.void)
            line(`_r${brs[brDepth - i[1] - 1]} = ${removeBrackets(vals.at(-1))}`);
          line(`goto j${brs[brDepth - i[1] - 1]}`);
          depth--;
          line(`}`, false);
...
int main(void) {
  ...
  out =
    platformSpecific(makeIncludes(winIncludes), makeIncludes(unixIncludes), false) +
    "\n" +
