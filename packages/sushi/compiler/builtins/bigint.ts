...
export const __Porffor_bigint_fromDigits = (
  negative: boolean,
  digits: i32[],
): bigint => {
...
export const __Porffor_bigint_fromNumber = (n: number): bigint => {
  if (!Number.isInteger(n) || !Number.isFinite(n))
    throw new RangeError("Cannot use non-integer as BigInt")
...
export const __Porffor_bigint_fromString = (n: string | bytestring): bigint => {
  ...
  while (i < len) {
    ...
    if (Porffor.fastOr(digit < 0, digit > 9))
...
export const __Porffor_bigint_toString = (
  x: number,
  radix: any,
): string | bytestring => {
  // todo: actually use bigint
  return __Number_prototype_toString(
    Math.trunc(__Porffor_bigint_toNumber(x)),
    radix,
  )
}

// todo: hook up all funcs below to codegen
export const __Porffor_bigint_add = (
  a: number,
  b: number,
  sub: boolean,
): bigint => {
