...
export const __ecma262_ToNumber = (argument: unknown): number => {
  ...
  if (
    Porffor.fastOr(
      ...
      Porffor.type(argument) === Porffor.TYPES.bigint,
    )
  )
    throw new TypeError("Cannot convert Symbol or BigInt to a number");

  // 3. If argument is undefined, return NaN.
  if (Porffor.type(argument) === Porffor.TYPES.undefined) return NaN;

  // 4. If argument is either null or false, return +0𝔽.
  if (Porffor.fastOr(argument === null, argument === false)) return 0;
...
export const __ecma262_ToIndex = (value: unknown): number => {
  ...
  if (Porffor.fastOr(integer < 0, integer > 9007199254740991))
...
export const __ecma262_ToString = (argument: unknown): any => {
  ...
  if ((Porffor.type(argument) | 0b10000000) === Porffor.TYPES.bytestring) return argument;

  // 2. If argument is a Symbol, throw a TypeError exception.
  if (Porffor.type(argument) === Porffor.TYPES.symbol)
  ...
  if (Porffor.fastAnd(Porffor.type(argument) === Porffor.TYPES.object, argument === 0))
    return "null";
  ...
  if (Porffor.type(argument) === Porffor.TYPES.number)
    return __Number_prototype_toString(argument, 10);

  // 8. If argument is a BigInt, return BigInt::toString(argument, 10).
  if (Porffor.comptime.flag`hasType.bigint`) {
    if (Porffor.type(argument) === Porffor.TYPES.bigint)
      return __Porffor_bigint_toString(argument, 10);
  }
