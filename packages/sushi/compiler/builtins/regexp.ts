...
export const __Porffor_regex_compile = (
  patternStr: bytestring,
  flagsStr: bytestring,
): RegExp => {
  ...
  while (patternPtr < patternEndPtr) {
    ...
    if (char == 92) {
      ...
      if (patternPtr >= patternEndPtr)
        throw new SyntaxError("Regex parse: trailing \\")
    ...
    if (classPtr) {
      ...
      if (!notEscaped) {
        if (char == 100)
          ...
          v =
            __Porffor_regex_hexDigitToValue(c1) * 4096 +
          ...
          if (patternPtr >= patternEndPtr) {
            ...
            if (
              (ctrlChar >= 65 && ctrlChar <= 90) ||
              (ctrlChar >= 97 && ctrlChar <= 122)
            ) {
              patternPtr++
              v = ctrlChar & 0x1f
            } else {
      ...
      if (
        ...
        if (
          ...
          patternPtr += 1
          if (patternPtr >= patternEndPtr)
            throw new SyntaxError("Regex parse: trailing \\ in range")
        }
    ...
    if (notEscaped) {
      ...
      if (char == 123) {
        ...
        if (!sawDigit)
          throw new SyntaxError("Regex parse: invalid {n,m} quantifier")
        if (patternPtr > patternEndPtr)
          throw new SyntaxError("Regex parse: unterminated {n,m} quantifier")
        ...
        for (let i: i32 = 1; i < n; i++) {
          for (let j: i32 = 0; j < atomSize; ++j) {
            Porffor.wasm.i32.store8(
              bcPtr + j,
              Porffor.wasm.i32.load8_u(lastAtomStart + j, 0, 0),
              0,
              0,
            )
          }
      ...
      // backreference: \1, \2, ...
      if (Porffor.fastAnd(char >= 49, char <= 57)) {
        // '1'-'9'
        lastAtomStart = bcPtr
...
export const __Porffor_regex_interpret = (
  regexp: RegExp,
  input: i32,
  isTest: boolean,
): any => {
  ...
  for (let i: i32 = lastIndex; i <= inputLen; i++) {
    ...
    interpreter: while (true) {
      ...
      switch (op) {
        ...
        case 0x0a: {
          ...
          if (arrIndex + 1 >= captures.length) {
            // reference to group that hasn't been seen
            pc += 2
          } else {
            ...
            if (capStart == -1 || capEnd == -1) {
              ...
              if (sp + capLen > inputLen) {
                ...
                for (let k: i32 = 0; k < capLen; k++) {
                  let c1: i32 = Porffor.wasm.i32.load8_u(
                    input + capStart + k,
                    0,
                    4,
                  )
    ...
    if (matched) {
      ...
      Porffor.array.fastPush(
        result,
        __ByteString_prototype_substring(input, matchStart, finalSp),
      )
    ...
    if (sticky) {
      // sticky, do not go forward in string
...
export const __RegExp_prototype_flags$get = (_this: RegExp) => {
  ...
  if (!Porffor.object.isObject(_this))
    throw new TypeError("this is a non-object")
...
export const RegExp = function (pattern: any, flags: any): RegExp {
  ...
  if (
    Porffor.type(patternSrc) !== Porffor.TYPES.bytestring ||
    Porffor.type(flagsSrc) !== Porffor.TYPES.bytestring
  ) {
...
export const __ByteString_prototype_match = (
  _this: bytestring,
  regexp: any,
) => {
  return __Porffor_regex_match(regexp, _this)
...
export const __Porffor_regex_matchAll = (regexp: any, input: any) => {
  ...
  if (!__RegExp_prototype_global$get(regexp))
    throw new TypeError("matchAll used with non-global RegExp")
...
export const __ByteString_prototype_matchAll = (
  _this: bytestring,
  regexp: any,
) => {
  return __Porffor_regex_matchAll(regexp, _this)
