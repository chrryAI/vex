      ...
      if (
        ...
        if (!endNotEscaped) {
          if (endChar === 100) endPredefClassId = 1;
            ...
            endChar =
              __Porffor_regex_hexDigitToValue(c1) * 16 + __Porffor_regex_hexDigitToValue(c2);
          } else if (endChar === 117) {
            // \u
            if (patternPtr + 3 >= patternEndPtr)
              throw new SyntaxError("Regex parse: invalid \\u escape");
    ...
    interpreter: while (true) {
      ...
      switch (op) {
        ...
        case 0x05: // start
          if (
            sp === 0 ||
            (multiline && sp > 0 && Porffor.wasm.i32.load8_u(input + sp, 0, 3) === 10)
          ) {
            pc += 1;
          } else {
            backtrack = true;
        ...
        case 0x06: // end
          if (
            sp === inputLen ||
            (multiline && sp < inputLen && Porffor.wasm.i32.load8_u(input + sp, 0, 4) === 10)
          ) {
            pc += 1;
          } else {
            backtrack = true;
