...
export const String = function (
  ...args: any[]
): string | bytestring | StringObject {
  ...
  for (let i: i32 = 0; i < len; i++) {
    ...
    if (codepoint <= 0xffff) {
      ...
      Porffor.wasm.i32.store16(
        Porffor.wasm`local.get ${out}` + outIndex * 2,
        codepoint,
        0,
        4,
      )
...
export const __String_prototype_charCodeAt = (_this: string, index: number) => {
  ...
  return Porffor.wasm.i32.load16_u(
...
export const __ByteString_prototype_charCodeAt = (
  _this: bytestring,
  index: number,
) => {
  const len: i32 = _this.length
