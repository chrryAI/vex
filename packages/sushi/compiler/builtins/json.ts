...
export const __Porffor_bytestring_bufferStr = (
  buffer: i32,
  str: bytestring,
): i32 => {
  ...
  while (ptr < endPtr) {
    Porffor.wasm.i32.store8(
      ptr++,
      Porffor.wasm.i32.load8_u(strPtr++, 0, 4),
      0,
      4,
    )
  }
...
export const __Porffor_bytestring_bufferChar = (
  buffer: i32,
  char: i32,
): i32 => {
...
export const __Porffor_bytestring_buffer2Char = (
  buffer: i32,
  char1: i32,
  char2: i32,
): i32 => {
...
export const __Porffor_json_serialize = (
  _buffer: i32,
  value: any,
  depth: i32,
  space: bytestring | undefined,
): i32 => {
  ...
  if (
    ...
    for (let i: i32 = 0; i < len; i++) {
      ...
      if (c < 0x20) {
        ...
        const h2: i32 = c & 0x0f
        buffer = __Porffor_bytestring_buffer2Char(
          buffer,
          h1 < 10 ? h1 + 48 : h1 + 55,
          h2 < 10 ? h2 + 48 : h2 + 55,
        ) // 0-9 or A-F
  ...
  if (
    ...
    if (Number.isFinite(value)) {
      return __Porffor_bytestring_bufferStr(
        buffer,
        __Number_prototype_toString(value, 10),
      )
    }
    ...
    for (const x of value as any[]) {
      if (hasSpace) {
        ...
        for (let i: i32 = 0; i < depth; i++)
          buffer = __Porffor_bytestring_bufferStr(buffer, space as bytestring)
      }
  ...
  if (space !== undefined) {
    if (
      ...
      if (space < 1) {
        ...
        for (let i: i32 = 0; i < space; i++)
          Porffor.bytestring.appendChar(spaceStr, 32)
...
export const __JSON_parse = (_: bytestring) => {
  ...
  const parseValue = (): any => {
      ...
      while (pos < len) {
        ...
        if (ch === 92) {
          ...
          if (esc === 34)
            ...
            Porffor.bytestring.appendChar(out, 92) // \\
          else if (esc === 47)
            Porffor.bytestring.appendChar(out, 47) // \/
          else if (esc === 98)
            Porffor.bytestring.appendChar(out, 8) // \b
          else if (esc === 102)
            Porffor.bytestring.appendChar(out, 12) // \f
          else if (esc === 110)
            Porffor.bytestring.appendChar(out, 10) // \n
          else if (esc === 114)
            Porffor.bytestring.appendChar(out, 13) // \r
          else if (esc === 116)
            Porffor.bytestring.appendChar(out, 9) // \t
          else if (esc === 117) {
            // \u
    ...
    if (c === 91) {
      ...
      if (pos < len && text.charCodeAt(pos) === 93) {
        // empty array
        pos++
        return arr
      }
    ...
    if (c === 123) {
      ...
      if (pos < len && text.charCodeAt(pos) === 125) {
        // empty object
        pos++
        return obj
      }
      ...
      while (true) {
        ...
        skipWhitespace()
        if (pos >= len || text.charCodeAt(pos) !== 58)
    ...
    if ((c >= 48 && c <= 57) || c === 45) {
      ...
      while (pos < len) {
        ...
        if (ch >= 48 && ch <= 57)
          ...
          pos++ // . e E
        else if ((ch === 43 || ch === 45) && pos > start + 1)
          pos++ // + - (not at start)
        else break
      }

      return ecma262.StringToNumber(
        __ByteString_prototype_slice(text, start, pos),
      )
    }
