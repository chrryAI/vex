...
export const __Number_prototype_toString = (
  _this: number,
  radix: number | any,
) => {
    ...
    if (i >= 1e21) {
      ...
      while (i > 0) {
        ...
        Porffor.wasm.i32.store8(
          Porffor.wasm`local.get ${digits}` + l,
          digit,
          0,
          4,
        )
...
export const __Number_prototype_toFixed = (
  _this: number,
  ...
  if (fractionDigits < 0 || fractionDigits > 100) {
    throw new RangeError(
      "toFixed() fractionDigits argument must be between 0 and 100",
    )
  }
...
export const __Number_prototype_toLocaleString = (_this: number) =>
  __Number_prototype_toString(_this, 10)

// fractionDigits: number|any for type check
export const __Number_prototype_toExponential = (
  _this: number,
  fractionDigits: number | any,
) => {
  if (!Number.isFinite(_this)) {
    if (Number.isNaN(_this)) return "NaN"
    if (_this == Infinity) return "Infinity"
...
export const __Number_parseInt = (input: any, radix: any): f64 =>
