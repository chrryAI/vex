...
export const run = (obj) => {
  ...
  const time = (id, msg) => {
    ...
    if (!starts[id]) {
      ...
      process.stdout.write(
        `\r${" ".repeat(50)}\r[${(performance.now() - starts[id]).toFixed(2)}ms] ${msg}\n`,
      )
    }
  }
  ...
  for (let i = 0; i < wasmFuncs.length; i++) {
    ...
    const invLocals = Object.keys(locals).reduce((acc, x) => {
      acc[locals[x].idx] = locals[x]
      return acc
    }, {})
    ...
    for (let j = 0; j < wasm.length; j++) {
      ...
      if (inst[0] === Opcodes.local_set || inst[0] === Opcodes.local_tee) {
        wasm.splice(
          ...
          ...(invLocals[inst[1]].type !== Valtype.f64
            ? [Opcodes.i32_from]
            : []),
          [Opcodes.call, importedFuncs.profileLocalSet],
        )
  ...
  localData = funcs.map((x) =>
    new Array(Object.keys(x.locals).length).fill(0).map(() => []),
  )
  ...
  for (let i = 0; i < localData.length; i++) {
    ...
    const consistents = localData[i].map((x, j) => {
      ...
      if (x.length === 0 || !x.every((y, i) => (i < 1 ? true : y === x[i - 1])))
