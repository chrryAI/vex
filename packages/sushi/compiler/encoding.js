...
export const number = (n, valtype = valtypeBinary) => {
  ...
  return [valtype === Valtype.i32 ? Opcodes.i32_const : Opcodes.i64_const, n]
}
...
export const encodeString = (str) =>
  unsignedLEB128(str.length).concat(codifyString(str))
export const encodeVector = (data) =>
  unsignedLEB128(data.length).concat(data.flat())
...
export const read_ieee754_binary64 = (buffer) =>
  new Float64Array(new Uint8Array(buffer).buffer)[0]
...
export const signedLEB128_into = (n, buffer) => {
  ...
  if (n >= 0 && n <= 8191)
    return buffer.push(128 + (n % 128), Math.floor(n / 128))
