    ...
    const rewriteWasm = (wasm) => {
      const str = JSON.stringify(
        ...
        .replace(
          /\["alloc","(.*?)",(.*?)\]/g,
          (_, reason, valtype) =>
            `[${+valtype === Valtype.i32 ? Opcodes.i32_const : Opcodes.f64_const},allocPage(_,'${reason}')]`,
        )
        .replace(
          /\["global",(.*?),"(.*?)",(.*?)\]/g,
        ...
        .replace(/\["str",(.*?),"(.*?)",(.*?)]/g, (_1, op, str, bytestring) =>
          op === "65"
            ? `...i32ify(makeString(_,"${str}",${bytestring === "true" ? 1 : 0}))`
            : `...makeString(_,"${str}",${bytestring === "true" ? 1 : 0})`,
        )
        .replace(
          /\["throw","(.*?)","(.*?)"\]/g,
        ...
        .replace(
          /\["get object","(.*?)"\]/g,
          (_, objName) => `...generateIdent(_,{name:'${objName}'})`,
        )
        .replace(
          /\[null,"typeswitch case start",\[(.*?)\]\],/g,
          (_, types) => `...t([${types}],()=>[`,
        )
...
${
  x.globalInits
    ? `globalInits:{${Object.keys(x.globalInits)
        .map((y) => `${y}:${rewriteWasm(x.globalInits[y])}`)
        .join(",")}},`
    : ""
}${x.data && Object.keys(x.data).length > 0 ? `data:${JSON.stringify(x.data)},` : ""}
