...
export default (
  ...
  const justLocals = Object.values(locals)
  ...
  for (let inst of wasm) {
    ...
    if (
      ...
      (inst[0] === Opcodes.end ||
        inst[0] === Opcodes.else ||
        inst[0] === Opcodes.catch_all ||
        inst[0] === Opcodes.catch)
    )
      depth--
    ...
    if (inst[0] === Opcodes.f64_const) {
      ...
      inst[0] === Opcodes.i32_load ||
      inst[0] === Opcodes.i64_load ||
      inst[0] === Opcodes.f64_load ||
      inst[0] === Opcodes.i32_store ||
      inst[0] === Opcodes.i64_store ||
      inst[0] === Opcodes.f64_store ||
      inst[0] === Opcodes.i32_store16 ||
      inst[0] === Opcodes.i32_load16_u
    ) {
    ...
    if (inst[0] === Opcodes.call || inst[0] === Opcodes.return_call) {
      ...
      if (callFunc)
        out += ` ;; $${callFunc.name} ${makeSignature(callFunc.params, callFunc.returns)}`
    ...
    if (
      inst[0] === Opcodes.local_get ||
      inst[0] === Opcodes.local_set ||
      inst[0] === Opcodes.local_tee
    ) {
...
export const highlightAsm = (asm) =>
  asm
    ...
    .replace(
      /(i(8|16|32|64)x[0-9]+|v128)(\.[^\s]*)?/g,
      (_) => `\x1B[34m${_}\x1B[0m`,
    )
    .replace(/(i32|i64|f32|f64|drop)(\.[^\s]*)?/g, (_) => `\x1B[36m${_}\x1B[0m`)
    .replace(
      /(return_call|call_indirect|call|br_if|br|return|rethrow|throw)/g,
      (_) => `\x1B[35m${_}\x1B[0m`,
    )
    .replace(
      /(block|loop|if|end|else|try|catch_all|catch|delegate)/g,
      (_) => `\x1B[95m${_}\x1B[0m`,
    )
    ...
    .replace(
      /;;.*$/gm,
      (_) => `\x1B[2m${_.replaceAll(/\x1B\[[0-9]+m/g, "")}\x1B[0m`,
    )
