...
import { signedLEB128_length, unsignedLEB128_length } from "./encoding.js"
...
export default (funcs, globals, tags, pages, data, noTreeshake = false) => {
  ...
  const time = (msg) => {
    ...
    console.log(
      `${" ".repeat(50)}\r[${(performance.now() - t).toFixed(2)}ms] ${msg}`,
    )
    t = performance.now()
  }
  ...
  if (Prefs.optLog)
    log(
      "assemble",
      `treeshake: using ${importFuncs.length}/${importedFuncs.length} imports`,
    )
  ...
  section(
    Section.type,
    unsignedLEB128_length(types.length) +
      types.reduce((acc, x) => acc + x.length, 0),
  )
  ...
  if (importFuncs.length > 0) {
    section(
      Section.import,
      unsignedLEB128_length(importFuncs.length) + importFuncs.length * 5,
    )
  ...
  if (pages.has("#func lut")) {
    ...
    for (let i = 0; i < indirectFuncs.length; i++) {
      ...
      bytes.push(
        ...new Uint8Array(
          new Int32Array([Math.min(name.length, bytesPerFunc - 3 - 4)]).buffer,
        ),
      )
    ...
    if (Prefs.debugFuncLut)
      log(
        "assemble",
        `func lut using ${bytes.length}/${pageSize * 2} (${bytesPerFunc} bytes per func)`,
      )
  ...
  if (globalsValues.length > 0) {
    section(
      Section.global,
      unsignedLEB128_length(globalsValues.length) +
        globalsValues.length * 4 +
        globalsValues.reduce(
          (acc, x) =>
            acc +
            (x.type === Valtype.f64 ? 8 : signedLEB128_length(x.init ?? 0)),
          0,
        ),
    )
  ...
  if (funcs.table) {
    section(
      Section.element,
      unsignedLEB128_length(indirectFuncs.length) +
        5 +
        indirectFuncs.reduce(
          (acc, x) => acc + unsignedLEB128_length(x.index - importDelta),
          0,
        ),
    )
    byte(1) // table index
  ...
  byte(Section.code)
  const codeSectionSizeOffset = offset,
    setCodeSectionSize = unsignedPost()

  unsigned(funcs.length)
  for (let i = 0; i < funcs.length; i++) {
    const funcSizeOffset = offset,
      setFuncSize = unsignedPost()
    ...
    for (let i = 0; i < x.wasm.length; i++) {
      ...
      if (op >= Opcodes.local_get && op <= Opcodes.global_set && o[1] > 127) {
        byte(op)
        unsigned(o[1])
        continue
      }
      ...
      if (
        op === Opcodes.call /* || o[0] === Opcodes.return_call */ &&
        o[1] >= importedFuncs.length
      ) {
        byte(op)
  ...
  section(
    Section.data,
    unsignedLEB128_length(data.length) +
      data.reduce(
        (acc, x) =>
          acc +
          (x.page != null
            ...
            : 1) +
          unsignedLEB128_length(x.bytes.length) +
          x.bytes.length,
        0,
      ),
  )
