...
export default (funcs, globals, tags, pages, data, noTreeshake = false) => {
  ...
  if (Prefs.optLog)
    log("assemble", `treeshake: using ${importFuncs.length}/${importedFuncs.length} imports`);
  ...
  section(
    Section.type,
    unsignedLEB128_length(types.length) + types.reduce((acc, x) => acc + x.length, 0),
  );
  ...
  if (pages.has("#func lut")) {
    ...
    for (let i = 0; i < indirectFuncs.length; i++) {
      ...
      bytes.push(
        ...new Uint8Array(new Int32Array([Math.min(name.length, bytesPerFunc - 3 - 4)]).buffer),
      );
    ...
    section(
      ...
      unsignedLEB128_length(globalsValues.length) +
        ...
        globalsValues.reduce(
          (acc, x) => acc + (x.type === Valtype.f64 ? 8 : signedLEB128_length(x.init ?? 0)),
          0,
        ),
    );
  ...
  if (funcs.table) {
    section(
      ...
      unsignedLEB128_length(indirectFuncs.length) +
        5 +
        indirectFuncs.reduce((acc, x) => acc + unsignedLEB128_length(x.index - importDelta), 0),
    );
    byte(1); // table index
    byte(0); // element type
    byte(Opcodes.i32_const);
    ...
    unsignedLEB128_length(data.length) +
      data.reduce(
        (acc, x) =>
          acc +
          (x.page != null
            ? 3 + signedLEB128_length(pages.allocs.get(x.page) ?? pages.get(x.page) * pageSize)
            : 1) +
