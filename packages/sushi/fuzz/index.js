...
if (cluster.isPrimary) {
  ...
  const duration =
    parseFloat(
      process.argv.find((x) => x.startsWith("--duration="))?.split("=")?.[1],
    ) || 60 // seconds
  const maxStatements =
    parseInt(
      process.argv
  ...
  console.log(
    `Max statements: ${maxStatements}, Max functions: ${maxFunctions}`,
  )
  ...
  const updateStats = () => {
    ...
    process.stdout.write(
      `\r\u001b[K${tab} | ${rate.toFixed(0)}/s | ${remaining.toFixed(1)}s left`,
    )
  }
  ...
  const cleanup = () => {
    ...
    if (failureFiles.length > 0) {
      ...
      if (failureFiles.length > 5)
        console.log(`  ... and ${failureFiles.length - 5} more`)
    }
  ...
  // Worker process
  let { maxStatements, maxFunctions, verbose, saveFailures, endTime } =
    JSON.parse(fs.readFileSync(workerDataPath, "utf8"))
  endTime ??= Infinity
