...
if (cluster.isPrimary) {
  ...
  const duration =
    parseFloat(process.argv.find((x) => x.startsWith("--duration="))?.split("=")?.[1]) || 60; // seconds
  const maxStatements =
    parseInt(process.argv.find((x) => x.startsWith("--max-statements="))?.split("=")?.[1], 10) ||
    20;
  const maxFunctions =
    parseInt(process.argv.find((x) => x.startsWith("--max-functions="))?.split("=")?.[1], 10) || 4;
  ...
  const cleanup = () => {
    ...
    if (errorCounts.size > 0) {
      ...
      sortedErrors.forEach(([error, count]) => {
        console.log(
          `\u001b[90m${((count / totalTests) * 100).toPrecision(2).padStart(6, " ")}%\u001b[0m \u001b[1m${error}\u001b[0m`,
        );
      });
    }
  ...
  // Worker process
  let { maxStatements, maxFunctions, verbose, saveFailures, endTime } = JSON.parse(
    fs.readFileSync(workerDataPath, "utf8"),
  );
  endTime ??= Infinity;
