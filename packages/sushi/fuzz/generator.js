...
const generators = {
  ...
  string: () => {
    ...
    if (Math.random() < 0.8) {
      ...
      return `'${Array.from(
        { length: len },
        () =>
          `\\u${Math.floor(Math.random() * 0x10000)
            .toString(16)
            .padStart(4, "0")}`,
      ).join("")}'`
    }
  ...
  array: (depth = 0) => {
    ...
    if (len === 0) return "[]"
    const items = Array.from({ length: len }, () =>
...
function generateExpression(depth = 0) {
  ...
  if (roll < 0.3) {
    ...
    return Math.random() < 0.5
      ? generators.array(depth)
      : generators.object(depth)
  }
}
...
function generateStatement(depth = 0, inFunction = false) {
  ...
  if (roll < 0.3) {
    ...
    // Variable assignment
    const varName =
      declaredVars[Math.floor(Math.random() * declaredVars.length)]
    const ops = [
      "=",
      "+=",
      "-=",
...
export {
  generateExpression as generate,
  generateStatement,
  generateFunction,
  generateProgram,
}
