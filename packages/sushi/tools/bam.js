...
async function detectBugs(filePath, code) {
  ...
  const typeMismatchPattern = /local\.(set|get|tee)\[0\] expected type (\w+), found (\w+)/g;
  let match;
  ...
  while ((match = testFailPattern.exec(code)) !== null) {
    bugs.push({
      ...
      line: code.substring(0, match.index).split("\n").length,
      suggestion: "Compute actual pass result first, then check currentlyFailing",
    });
  }
...
async function analyzeBugPatterns() {
  ...
  if (result?.data) {
    for (const row of result.data) {
      ...
      console.log(`   ${type}: ${count} occurrences (avg line: ${Math.round(avgLine)})`);
    }
  } else {
...
async function scanDirectory(dirPath, extensions = [".js", ".ts", ".jsx", ".tsx"]) {
  const files = [];
