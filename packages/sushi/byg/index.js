...
const printLine = (
  ...
  console.log(
    `\x1b[${breakpoint ? (selected ? "43" : "103") : selected ? "47" : "100"}m\x1b[${selected || breakpoint ? "30" : "97"}m${number.toFixed(0).padStart(4, " ")}\x1b[${breakpoint ? (selected ? "33" : "93") : selected ? "37" : "90"}m\x1b[${current ? "47" : "40"}m▌ \x1b[${current ? "47" : "40"}m\x1b[${current ? "30" : "37"}m${current ? noAnsi(line) : line}\x1b[0K`,
  )
}
...
const box = (
  ...
  if (title)
    process.stdout.write(
      `\x1b[${y};${x}H\x1b[0m\x1b[${color[3]}m▐\x1b[${color[4]}m\x1b[${color[2]}m\x1b[1m${" ".repeat(((width - title.length) / 2) | 0)}${title}${" ".repeat(width - (((width - title.length) / 2) | 0) - title.length)}\x1b[0m\x1b[${color[3]}m▌`,
    )

  // content
  process.stdout.write(
    `\x1b[${y + (title ? 1 : 1)};${x + 1}H\x1b[${color[1]}m\x1b[${color[2]}m${content.join(`\x1b[1B\x1b[${x + 1}G`)}`,
  )
}
  ...
  return (_paused, currentLine, text, boxes = []) => {
    ...
    const draw = () => {
      ...
      if (tooManyLines) {
        ...
        for (
          let i = Math.max(0, beforePadding);
          i < Math.max(0, beforePadding) + (termHeight - 1);
          i++
        ) {
          printLine(
            lines[i],
      ...
      process.stdout.write(
        `\x1b[${termHeight};1H\x1b[105m\x1b[37m${text}${" ".repeat(termWidth - plainControlInfo.length - noAnsi(text).length - 1)}${controlInfo} \x1b[0m`,
      )
    }
    ...
    while (true) {
      ...
      switch (char.toLowerCase()) {
        ...
        case "b": {
          if (!lastSpecial) {
            // b pressed normally
            breakpoints[currentLine + scrollOffset] =
              !breakpoints[currentLine + scrollOffset]
            draw()
        ...
        case "6": {
          ...
          scrollOffset += Math.min(
            lines.length - (scrollOffset + currentLine) - 1,
            termHeight - 1,
          )
          draw()
          break
