...
export type BookmarkStylesHook = {
  [K in keyof typeof BookmarkStyleDefs]: (typeof BookmarkStyleDefs)[K] extends {
    base: any
  }
...
export const useBookmarkStyles = (): BookmarkStylesHook => {
  // Call all hooks upfront in a stable order (Rules of Hooks compliant)
  const styleResults: Partial<Record<keyof typeof BookmarkStyleDefs, any>> = {}

  // Use Object.keys to ensure consistent iteration order across environments
  const keys = Object.keys(BookmarkStyleDefs) as Array<
    keyof typeof BookmarkStyleDefs
  >
  ...
  return styleResults as BookmarkStylesHook
}
