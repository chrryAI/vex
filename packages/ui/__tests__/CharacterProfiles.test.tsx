...
vi.mock("../CharacterProfile", () => ({ default: () => <div data-testid="character-profile" /> }))
vi.mock("../ConfirmButton", () => ({ default: ({ children, onConfirm }: any) => <button data-testid="confirm-button" onClick={onConfirm}>{children}</button> }))
...
describe("CharacterProfiles", () => {
    ...
    it("displays character profiles when enabled and present", async () => {
        ...
        expect(container.querySelector("[data-testid='character-profile']")).toBeTruthy()
    })
    ...
    it("displays enable button when disabled", async () => {
        ...
        expect(container.querySelector("[data-testid='enable-character-profiles']")).toBeTruthy()
    })
...
// Add lib mock at top
vi.mock("../lib", () => ({
    updateUser: vi.fn(() => Promise.resolve({ id: "user-1", characterProfilesEnabled: false }))
}))
