...
vi.mock("../Modal", () => ({ default: ({ children, isModalOpen }: any) => isModalOpen ? <div data-testid="modal">{children}</div> : null }))
...
vi.mock("../App", () => ({ default: () => <div data-testid="app-suggestions" /> }))
vi.mock("../Image", () => ({ default: () => <div data-testid="image" /> }))
vi.mock("../MoodSelector", () => ({ default: () => <div data-testid="mood-selector" /> }))
...
describe("Chat", () => {
    ...
    it("handles input change", async () => {
        ...
        if (textarea) {
            await act(async () => {
                // Set value
                Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value')?.set?.call(textarea, "Hello AI");
    ...
    it("opens agent selection modal when agent button is clicked", async () => {
        ...
        const agentButton = container.querySelector("[data-testid='agent-select-button']")
    ...
    it("toggles web search", async () => {
        ...
        const webSearchButton = container.querySelector("[data-testid='web-search-button-disabled']")
        expect(webSearchButton).toBeTruthy()

        if (webSearchButton) {
            await act(async () => {
                webSearchButton.dispatchEvent(new MouseEvent("click", { bubbles: true }))
            })
