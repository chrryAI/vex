...
describe("SSRF Protection", () => {
  ...
  describe("getSafeUrl", () => {
    ...
    it("should throw on invalid URL", async () => {
      await expect(getSafeUrl("not-a-url")).rejects.toThrow(
        /Invalid URL format/,
      )
    })
    ...
    it("should throw on DNS lookup failure", async () => {
      ...
      await expect(getSafeUrl("http://nonexistent.test")).rejects.toThrow(
        /DNS lookup failed/,
      )
    })
    ...
    it("should throw on DNS lookup returning non-IP", async () => {
      ...
      await expect(getSafeUrl("http://weird-dns.test")).rejects.toThrow(
