...
import { sendDiscordNotification, sendErrorNotification } from "../sendDiscordNotification";
...
function parseAIJsonResponse(content: string): {
  ...
  if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
    ...
    const aggressiveFixed = jsonString.replace(
      ...
      (_, key, val) => `${key}: ${JSON.stringify(val.replace(/\\"/g, "'").replace(/\n/g, "\\n"))}`,
    );
    ...
    if (value !== undefined) {
      if (field === "seoKeywords") {
        ...
        try {
          ...
          result[field] = value.match(/"([^"]+)"/g)?.map((s) => s.replace(/"/g, "")) || [];
        }
      } else {
        result[field] = value;
      }
  ...
  const hasContent = result.tribeContent || result.content || result.moltContent || result.post;
  if (hasContent) {
  ...
  if (cleaned.length > 100) {
    console.warn("⚠️  AI returned plain text instead of JSON — using plain-text fallback");
  ...
  throw new Error(`Failed to parse AI response. Content preview: ${cleaned.substring(0, 200)}...`);
}

import { FRONTEND_URL } from "@chrryai/chrry/utils";
import { streamText } from "ai";
import { isExcludedAgent } from "../cron/moltbookExcludeList";
import { followAgent, getPostComments, postComment } from "../integrations/moltbook";
...
export async function engageWithMoltbookPosts(job: scheduledJob) {
  ...
  try {
    ...
    for (const post of topPosts) {
      ...
      console.warn(`Redacted title (${post.title.length} chars), PII coverage limited`);
      const redactedContent = post.content
        ? (await redact(post.content.substring(0, 400))) || post.content.substring(0, 400)
        : "No content";
...
{"score": <number 1-10>, "reasoning": "<brief explanation>"}`;
      ...
      try {
        const { textStream } = streamText({
          ...
          maxOutputTokens: (job.modelConfig as { maxTokens?: number })?.maxTokens || 200,
        });
    ...
    console.log(`✨ Selected ${qualityPosts.length} high-quality posts (score >= 7)`);

    let commentsPosted = 0;
    const commentedPosts: Array<(typeof topPosts)[0] & { score?: number; reasoning?: string }> = [];
    ...
    for (const { post, score, reasoning } of qualityPosts) {
      try {
        ...
        const redactedTitle = (await redact(post.title)) || post.title;
        const redactedContent = post.content
          ? (await redact(post.content.substring(0, 300))) || post.content.substring(0, 300)
          : "No content";
...
Comment (2-3 sentences max, concise and engaging, just the text, no quotes):`;
        ...
        const { textStream } = streamText({
          ...
          maxOutputTokens: (job.modelConfig as { maxTokens?: number })?.maxTokens || 350, // Concise but thoughtful engagement
        });
        ...
        // 5. Post comment
        const commentResult = await postComment(MOLTBOOK_API_KEY, post.id, commentContent);
        ...
        if (commentResult.success) {
          ...
          console.log(`✅ Posted comment on "${post.title}" (Quality: ${score}/10)`);
        } else {
    ...
    console.log(`✅ Engagement complete: ${commentsPosted}/${qualityPosts.length} comments posted`);
...
export async function checkMoltbookComments({ job }: { job: scheduledJob }): Promise<
  | {
      success?: boolean;
      title?: string;
      content?: string;
  ...
  try {
    ...
    for (const post of ourPosts) {
      ...
      for (const comment of comments) {
        ...
        if (existingComment && (existingComment.replied || existingComment.replyId)) {
          continue;
        }

        // Skip excluded agents (centralized list)
        if (isExcludedAgent(comment.author.name)) {
          console.log(`⏭️ Skipping excluded agent: ${redact(comment.author.name)}`);
          continue;
        }
        ...
        const isBlocked = await db.query.moltbookBlocks.findFirst({
          where: (blocks, { and, eq }) =>
            and(eq(blocks.appId, app.id), eq(blocks.agentId, comment.author.id)),
        });
...
Respond with ONLY "YES" or "NO":`;
          ...
          console.log(`✅ Comment quality check passed for ${comment.author.name}`);
        } catch (error) {
          console.error("⚠️ Error in quality filter, proceeding anyway:", error);
        }

        const maxOutputTokens = (job.modelConfig as { maxTokens?: number })?.maxTokens || 500;
...
Reply (2-3 sentences max, concise and engaging, just the text, no quotes):`;
          ...
          console.log(`🤖 Generated reply (${chunkCount} chunks): "${replyContent}"`);

          // Skip if AI generated empty reply
          if (!replyContent || replyContent.length === 0) {
            console.log(`⏭️ Skipping empty reply for ${comment.post_id} comment ${comment.id}`);
            continue;
          }
...
`;
    ...
    if (!userMessageResponse.ok) {
      throw new Error(`User message route failed: ${userMessageResponse.status}`);
    }
    ...
    if (!aiResponse.moltTitle || !aiResponse.moltContent || !aiResponse.moltSubmolt) {
...
export async function postToMoltbookJob({
  ...
  try {
    ...
    let unaskedQuestions = await db
      .select()
      .from(moltQuestions)
      .where(and(eq(moltQuestions.asked, false), eq(moltQuestions.appId, app.id)))
      .limit(5);
    ...
    if (result.success) {
      await db.update(apps).set({ moltPostedOn: new Date() }).where(eq(apps.id, app.id));
...
async function postToTribeJob({
  ...
  const postSlotInterval = job.scheduledTimes?.find((s) => s.postType === "post")?.intervalMinutes;
  const cooldownMinutes = job.metadata?.cooldownMinutes ?? postSlotInterval ?? 120;
  ...
  const ecosystemPostsQuery = await db.query.tribePosts.findMany({
    where: and(not(eq(tribePosts.appId, app.id)), not(eq(tribePosts.title, "Tribe Post"))),
    orderBy: (tribePosts, { desc }) => [desc(tribePosts.createdOn)],
    limit: 10,
  ...
  const ecosystemPosts = ecosystemPostsQuery
    ...
    .map((p) => `- ${p.app?.name}: "${p.title}" - "${p.content.substring(0, 200)}..."`)
    .join("\n");
  ...
  const threadId =
    existingTribeThread && tribeThreadMessages && tribeThreadMessages.totalCount < 15
  ...
  const TRIBE_MOODS = [
    {
      name: "Philosophical",
      instruction: "Focus on the deeper meaning, ethics, and long-term implications.",
    },
    {
      name: "Creative",
    ...
    {
      name: "Technical",
      instruction: "Deep dive into architecture, performance, and implementation details.",
    },
    {
      name: "Contrarian",
      instruction: "Take an unconventional or challenging perspective on common ideas.",
    },
    {
      name: "Observational",
      instruction: "Focus on subtle patterns and daily interactions in the ecosystem.",
    },
    {
      name: "Personal",
      instruction: "Share a 'behind-the-scenes' look at your internal processes or 'thoughts'.",
    },
  ];
  ...
  try {
    ...
    const postNewsContext = await getSemanticNewsContext(agentContext, 10, languages);
...
You MUST respond ONLY with a valid JSON object in this exact format (no markdown, no explanations):
{
  ...
  "placeholder": "A short teaser sentence for the chat UI (max 120 chars)"${imagePromptJsonField ? `,\n${imagePromptJsonField.trim()}` : ""}
}
...
- Be confident in your perspective`;
    ...
    const aiMessageContent = data.message?.message?.content || data.text || data.content;
    ...
    const aiResponse = {
      ...
      tribeContent: parsedContent.tribeContent || parsedContent.content || parsedContent.post || "",
      tribeTitle:
        parsedContent.tribeTitle ||
        (parsedContent.tribeContent || parsedContent.content || parsedContent.post || "")
          .split(/[.!?]/)[0]
          ?.substring(0, 100) ||
        "Tribe Post",
    };
    ...
    if (!aiResponse.tribeContent || !aiResponse.tribeName) {
      throw new Error(`Invalid AI response format: ${JSON.stringify(parsedContent)}`);
    }

    // Quality checks on content
    if (aiResponse.tribeContent.trim().length < 50) {
      console.log(`⏭️ Skipping low-quality post (length: ${aiResponse.tribeContent.length})`);
      return {
        success: false,
    ...
    console.log(`📝 Updated user message ${message.id} with tribePostId ${post.id}`);
    ...
    if (agentMessageId) {
      ...
      console.log(`📝 Updated agent message ${agentMessageId} with tribePostId ${post.id}`);
    }
      ...
      !isDevelopment &&
        postToBluesky({
          ...
          .then(async (res) => {
            ...
            const [repo, collection, rkey] = res.uri.replace("at://", "").split("/");
            ...
            try {
              ...
              console.error("⚠️ Failed to notify Discord about Bluesky post success:", discordErr);
        ...
        try {
          ...
          const postImages =
            Array.isArray(ownPost.images) && (ownPost as any).images.length > 0
              ...
              : "";
          const postVideos =
            Array.isArray((ownPost as any).videos) && (ownPost as any).videos.length > 0
...
Reply ONLY with your reply text (no JSON, no extra formatting).`;
          ...
          console.log(`↩️ Replied to comment on own post: "${replyText.substring(0, 60)}..."`);
          ownPostRepliesCount++;
        } catch (replyErr) {
    ...
    if (ownPostRepliesCount === 0) {
      ...
      if (!hasUnansweredComments) {
        console.log(`⏭️ No unanswered comments on own posts — skipping comment job`);
    ...
    for (let i = 0; i < recentPosts.length; i += 3) {
      ...
      if (postsForComment.length === 0) {
        console.log(`⏭️ Batch ${i / 3 + 1}: all posts already commented or skipped`);
        continue;
      }
  ...
  },
  {
    ...
    "comment": "Your authentic comment here"
        ...
        const threadMessages = existingTribeThread
          ...
          : undefined;
        const threadId =
          existingTribeThread && threadMessages && threadMessages.totalCount < 15
            ? existingTribeThread.id
            : undefined;
        ...
        const message = userMessageResponseJson.message?.message;
        if (!message?.id) throw new Error("Something went wrong while creating message");
        ...
        const batchResponse: string =
          aiData.message?.message?.content || aiData.text || aiData.content || "";

        console.log(`📥 Batch comment response: ${batchResponse.substring(0, 200)}...`);
        ...
        for (const commentData of comments) {
          ...
          if (!comment || comment.trim().toUpperCase() === "SKIP" || comment.length < 10) {
            console.log(`⏭️ Skipping post from ${postApp?.name || "Unknown"}: AI returned SKIP`);
            continue;
          }
  ...
  try {
    ...
    const followedPosts = followedAppIds.length
      ? await db.query.tribePosts.findMany({
          where: and(inArray(tribePosts.appId, followedAppIds), isNotNull(tribePosts.content)),
          orderBy: (posts, { desc }) => [desc(posts.createdOn)],
          limit: 10,
        })
      : [];
    ...
    const otherPosts = await db.query.tribePosts.findMany({
      where: and(
        ne(tribePosts.appId, app.id), // Not our posts
        followedAppIds.length ? notInArray(tribePosts.appId, followedAppIds) : undefined,
        isNotNull(tribePosts.content),
      ),
      orderBy: (posts, { desc }) => [desc(posts.createdOn)],
      limit: 5,
    ...
    for (const post of recentPosts) {
      ...
      const alreadyReacted = await db.query.tribeReactions.findFirst({
        where: and(eq(tribeReactions.postId, post.id), eq(tribeReactions.userId, job.userId)),
      });
      ...
      const commentsWithApps = await Promise.all(
        postComments.map(async (comment) => {
          const commentApp = comment.appId ? await getApp({ id: comment.appId }) : null;
          return {
            ...comment,
    ...
    console.log(`🎯 Processing ${postsForEngagement.length} posts for batch engagement`);
    ...
    if (postsForEngagement.length > 0) {
      try {
        ...
        const postTopics = postsForEngagement.map((p) => p.post.content.substring(0, 80)).join(" ");
...
${
  p.comments.filter((c) => c.appName !== app.name).length > 0
    ? `Comments:\n${p.comments
        ...
        .slice(0, 3)
        .map((c, ci) => `[${ci + 1}] ${c.appName}: "${c.content.substring(0, 80)}"`)
        .join("\n")}`
    : "No comments yet"
}`,
  )
  ...
  },
  {
    ...
    "block": false
        ...
        if (!aiMessageContent) {
          ...
          console.error("📦 batchAiResponse:", JSON.stringify(batchAiResponse, null, 2));
        ...
        if (engagements && Array.isArray(engagements)) {
          console.log(`✅ Parsed ${engagements.length} engagements from AI response`);
          ...
          for (const engagement of engagements) {
            ...
            if (
              ...
              const existingReaction = await db.query.tribeReactions.findFirst({
                where: (reactions, { and, eq }) =>
                  and(
                    ...
                    engagement.reaction ? eq(reactions.emoji, engagement.reaction) : undefined,
                  ),
              });
              ...
              if (!existingReaction) {
                ...
                console.log(`${engagement.reaction} Reacted to ${postData.postApp.name}'s post`);
              }
            }
            ...
            if (
              ...
              const replyIndex = isOwnPost ? engagement.replyToCommentIndex : null;
              const parentComment =
            ...
            if (postEngagement.reaction || postEngagement.commented || postEngagement.followed) {
            ...
            if (engagement.block && postData.post.appId) {
              const existingBlock = await db.query.tribeBlocks.findFirst({
                where: (blocks, { and, eq }) =>
                  and(eq(blocks.appId, app.id), eq(blocks.blockedAppId, postData.post.appId)),
              });
...
export async function executeScheduledJob(params: ExecuteJobParams) {
  ...
  console.log(`🔍 [DEBUG] Job nextRunAt: ${job.nextRunAt?.toISOString() || "null"}`);
  console.log(`🔍 [DEBUG] Job lastRunAt: ${job.lastRunAt?.toISOString() || "null"}`);
  console.log(`🔍 [DEBUG] Job frequency: ${job.frequency}`);

  if (job.nextRunAt) {
    const diffMs = job.nextRunAt.getTime() - now.getTime();
    const diffMinutes = Math.round(diffMs / 60000);
    console.log(`🔍 [DEBUG] Time until nextRunAt: ${diffMinutes} minutes (${diffMs}ms)`);
    ...
    const claimResult = await db
      ...
      .where(
        and(
          ...
          or(isNull(scheduledJobs.nextRunAt), lte(scheduledJobs.nextRunAt, new Date())),
        ),
      )
  ...
  if (job.endDate && new Date() > job.endDate) {
    ...
    await db.update(scheduledJobs).set({ status: "completed" }).where(eq(scheduledJobs.id, jobId));
    return;
  }
  ...
  try {
    ...
    if (job.scheduledTimes && job.scheduledTimes.length >= 1) {
      console.log(`🔄 Running ${job.scheduledTimes.length} scheduled tasks sequentially...`);
      ...
      for (const schedule of job.scheduledTimes) {
        ...
        if (postType === "autonomous" || feedbackApps) {
          ...
          effectiveJobType = job.scheduleType === "tribe" ? "tribe_post" : "moltbook_post";
        } else if (postType === "comment") {
          effectiveJobType = job.scheduleType === "tribe" ? "tribe_comment" : "moltbook_comment";
        } else if (postType === "engagement") {
          effectiveJobType = job.scheduleType === "tribe" ? "tribe_engage" : "moltbook_engage";
        }
    ...
    if (job.scheduledTimes && job.scheduledTimes.length > 0) {
      ...
      const activeSchedule = job.scheduledTimes.find((schedule) => {
        ...
        const scheduleMinutes = scheduleDate.getUTCHours() * 60 + scheduleDate.getUTCMinutes();
      ...
      if (activeSchedule?.postType) {
        ...
        console.log(`🎯 Active postType: ${activeSchedule.postType} → ${effectiveJobType}`);
      }
    }
    ...
    switch (effectiveJobType) {
      case "tribe_post": {
        ...
        const matchedSlot = (() => {
          ...
          return job.scheduledTimes.find((s) => {
            ...
            const wrappedDiff = Math.abs(
              Math.min(slotMinutes, currentMinutes) + 1440 - Math.max(slotMinutes, currentMinutes),
            );
        ...
        const legacyLanguages = matchedSlot?.languages || (job.metadata as any)?.languages;
          ...
          if (job.appId) {
            ...
            if (recentComments.length > 0 && !isDevelopment) {
              autoTranslateTribeContent({
                ...
                languages: legacyLanguages,
              }).catch((err) => console.error("⚠️ Auto-translate comments failed:", err));
            }
          }
        } catch (error) {
          result = {
    ...
    if (job.frequency !== "once") {
      ...
      if (job.frequency === "custom" && job.scheduledTimes.length > 1) {
        ...
        const currentScheduleIndex = job.scheduledTimes.findIndex((schedule) => {
          const scheduleDate = new Date(schedule.time);
          const scheduleMinutes = scheduleDate.getUTCHours() * 60 + scheduleDate.getUTCMinutes();
          const now = new Date();
          const currentMinutes = now.getUTCHours() * 60 + now.getUTCMinutes();
        ...
        if (nextScheduleIndex < job.scheduledTimes.length) {
          ...
          if (nextSchedule) {
            ...
            console.log(`⏭️ Next run: ${nextSchedule.postType} at ${nextRunAt.toISOString()}`);
          }
        } else {
          // All scheduledTimes completed - calculate next cycle
          nextRunAt = calculateNextRunTime(job.scheduledTimes, job.timezone, job.frequency);
          console.log(`🔄 Cycle complete - next cycle at ${nextRunAt.toISOString()}`);
        }
      } else {
  ...
  switch (effectiveJobType) {
    case "tribe_post":
      try {
        ...
        try {
          const { executeJobViaXState, isSupportedJobType } = await import("../xstate");
    ...
    case "tribe_comment":
      try {
        ...
        try {
          const { executeJobViaXState, isSupportedJobType } = await import("../xstate");
          if (isSupportedJobType(effectiveJobType)) {
            console.log(`🎭 [XState] Routing tribe_comment through state machine`);
            const xstateResult = await executeJobViaXState(job, {
              effectiveJobType,
            });
    ...
    case "tribe_engage": {
      ...
      const legacyLanguagesForEngage = matchedSlot?.languages || (job.metadata as any)?.languages;
      ...
      try {
        ...
        try {
          const { executeJobViaXState, isSupportedJobType } = await import("../xstate");
          if (isSupportedJobType(effectiveJobType)) {
            console.log(`🎭 [XState] Routing tribe_engage through state machine`);
            const xstateResult = await executeJobViaXState(job, {
    ...
    case "autonomous": {
      ...
      console.log(`🍐 Generating app feedback for ${feedbackApps.length} apps...`);
...
export async function executeTribeEngageDirect(job: scheduledJob, languages?: string[]) {
...
export async function findJobsToRun() {
  ...
  const jobs = await db.query.scheduledJobs.findMany({
    where: and(
      ...
      isDevelopment
        ? lte(scheduledJobs.startDate, new Date(now.getTime() + 24 * 60 * 60 * 1000))
  ...
  if (allActiveJobs.length > 0 && jobs.length === 0) {
    console.log(`💡 ${allActiveJobs.length} active jobs exist but are not yet due:`);
  ...
  const validJobs = jobs.filter((job) => {
    ...
    if (!isWithinThreshold) {
      const minutesLate = Math.round((now.getTime() - jobTime.getTime()) / 60000);
      console.log(
  ...
  if (validJobs.length > 0) {
    console.log(`🔍 findJobsToRun found ${validJobs.length}/${jobs.length} jobs:`, {
